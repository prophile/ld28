...
final class ObstacleRenderer {
    public static void renderObstacles(WorldRenderer worldRenderer) {
        Iterator<Obstacle> obstacles = worldRenderer.getWorld()
                .obstaclesBetween(0.0, 1.0);
        ...
        while (obstacles.hasNext()) {
            ...
            worldRenderer.drawSprite("" + ob.getValue(), eyes.x, eyes.y, 32.0f,
                    ob.getValue() == 1 ? worldRenderer.getRequest()
                            .getColourScheme().getForeground() : worldRenderer
                            .getRequest().getColourScheme().getBad());
