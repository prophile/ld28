...
public class BezierIteration {
    ...
    public static void bezierStep(double[] points, int st, int it) {
        ...
        int ofs = data * 2; // where the new data points go
    ...
    public static void coordinatesToSegments(double[] x, double[] y, Segment[] s) {
        ...
        sLength = Math.sqrt(Math.pow(x[counter] - x[0], 2)
                + Math.pow(y[counter] - y[0], 2));
        v1[0] = x[0] - x[counter];
        v1[1] = y[0] - y[counter];
        ...
        s[counter] = new Segment(sLength, Angle.degrees(sAngle));

        // calculate other segments
        while (counter > 0) {
            counter = counter - 1;
            // calculate length
            sLength = Math.sqrt(Math.pow(x[counter] - x[counter + 1], 2)
                    + Math.pow(y[counter] - y[counter + 1], 2));
